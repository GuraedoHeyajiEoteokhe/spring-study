...
## 리뷰어 : 양승재
현재 단위프로젝트를 하며 자주 사용하는 어노테이션들이 이번 정리에 많이 들어있는것같습니다.
Mock을 사용해서 redis를 사용할때 test코드를 작성하며 redis를 이용하지 않을 수 있어 중요한것같습니다.
JPA에서는 간단한 쿼리문만 작성해주기때문에 join이상을 하려면 sql을 작성해야해 sql문도 조금 더 공부해야한다고 생각합니다.
